...
def decode_setup_bytes(data: bytearray) -> dict:
    ...
    _, act_thr, deact_thr, deact_time_units, adv_interval_byte, _, _, conn_interval_units = struct.unpack("<BBBBBBBB", data)
...
async def main():
    # If you don’t know your device’s address ahead of time, you can scan for Pam devices by name.
    # Uncomment the following block to scan for ~5 seconds and print all devices whose name contains “Pam”:
    #
    # print("Scanning for BLE devices (5 seconds)...")
    # devices = await BleakScanner.discover(timeout=5.0)
    # for d in devices:
    #     if d.name and "Pam" in d.name:
    #         print(f"  → {d.name} ({d.address})")
    #
    # Then set TARGET_DEVICE_ADDRESS to the printed address.

    async with BleakClient(TARGET_DEVICE_ADDRESS) as client:
        if not client.is_connected:
            print(f"Failed to connect to {TARGET_DEVICE_ADDRESS}")
            return
        print(f"Connected to {TARGET_DEVICE_ADDRESS}")
        ...
        decoded = decode_setup_bytes(raw)
        ...
        # decoded2 = decode_setup_bytes(raw2)
