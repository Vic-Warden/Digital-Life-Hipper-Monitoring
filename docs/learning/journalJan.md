...
What I learned is that for us to perform a similar test we need to also take two (or more) Hipper devices and get them to stick together as close as possible. After that it is time for a series of tests, such as walking, running etc. Any and all movements are acceptable. Finally after collecting data it is time to start comparing datasets of unique Hipper devices to eachother. Do the Hipper devices give nearly similar data according to the graphs that are present in the already existing Research report? That, we will find out in another User Story. 
