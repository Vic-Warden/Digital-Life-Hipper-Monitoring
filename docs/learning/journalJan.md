...
* Apparently Power BI Desktop allows users to efficiently import and combine multiple data files stored in a local folder, such as .csv reports. By connecting to a folder as a data source, Power BI can automatically detect and list all files within it. Using a sample file as a template, it enables consistent transformations—like removing headers or changing column types—across all files. I researched this by looking up a few things online to do this automatically. I then tried it myself and it immediately worked without issue! The file of the tutorial can be found at: `frontend/powerbi/`
